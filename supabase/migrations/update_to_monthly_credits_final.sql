-- FINAL Migration: Update to Monthly Credit System with No Rollover
-- This migration is designed to work AFTER complete_setup.sql has been run
-- It safely updates the existing system to monthly credit limits

-- Step 1: Add new columns to profiles table (safe with IF NOT EXISTS)
...
-- Step 5: Drop all existing functions that will be replaced (in correct order)
...
DROP FUNCTION IF EXISTS public.process_successful_payment(UUID, TEXT, TEXT, TEXT, INTEGER, TEXT, TEXT, TEXT, INTEGER) CASCADE;

-- Step 6: Create helper function to calculate next month's reset date
...
-- Step 7: Create function to reset monthly credits for a user
...
-- Step 8: Create new use_credits function with monthly limits
...
-- Step 9: Create new process_successful_payment function for monthly credit system
...
-- Step 10: Create function to process monthly credit resets (runs daily via cron)
...
-- Step 12: Update function comments
...
DO $$
BEGIN
  RAISE NOTICE '=== MONTHLY CREDIT SYSTEM MIGRATION COMPLETED ===';
  RAISE NOTICE '';
  RAISE NOTICE 'Successfully updated to monthly credit limits with no rollover!';
  RAISE NOTICE '';
  RAISE NOTICE 'New Credit System:';
  RAISE NOTICE '- Yearly Plan (₹999): 150 credits/month, no rollover';
  RAISE NOTICE '- Monthly Plan (₹1499): 100 credits/month, no rollover';
  RAISE NOTICE '- Free Plan: 0 credits';
  RAISE NOTICE '- Unused credits are lost at month end';
  RAISE NOTICE '';
  RAISE NOTICE 'Database Changes:';
  RAISE NOTICE '- Added monthly credit tracking columns';
  RAISE NOTICE '- Updated all functions for monthly limits';
  RAISE NOTICE '- Migrated existing users safely';
  RAISE NOTICE '';
  RAISE NOTICE 'IMPORTANT - Next Steps:';
  RAISE NOTICE '1. Update /api/payment/verify/route.ts to use p_credits_per_month parameter';
  RAISE NOTICE '2. Set up daily cron: SELECT public.process_monthly_credit_resets();';
  RAISE NOTICE '3. Test credit usage: SELECT * FROM public.use_credits(user_id, 10, ''test'');';
  RAISE NOTICE '';
